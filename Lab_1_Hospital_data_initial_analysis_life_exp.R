...
# Methods and Tools for Health Statistics - Lab 3 
...
## Life expectancy: Demography package ----                                                                                                               
# It includes functions for demographic analysis including life table calculations,                                                     
# Lee-Carter modelling and functional data analysis of mortality rates.It also includes some special R objects defined as demogdata,   
# which we will extract and analyse                                                                                                     

rm(list=ls())

install.packages("demography")

library(demography)     # In order to use the function from demography package we have to load the package 

# A simple way to check what a package contains and access to the objects of a determinate package. 

demography::fr.mort

# Life Table Data Management

## This is the dataset object of class "demogdata". It contains the mortality rates of the total, female and male population of France from 1816 to 2006

class(fr.mort)                              # Show class: this is a special R object
names(fr.mort)                              # List the variables
fr.mort$year                                # Show content of one variable. 

p<-as.data.frame(fr.mort$rate)              # Turn "rate" columns into a data frame. Note: rate is not a vector but a table. 
                                         # Show variable names

m_x<-p$total.2000                           # Assign total.2000 vector from p dataframe to m_x.
x<-as.data.frame(fr.mort$age)               # Assign age vector from fr.mort object to x and turn AGE columns into a data frame
names(x)<-c("age")                          # The name of the variable assigned automatically is fr.mort$age, we rename the column of x, calling it x. 



life_table<-as.data.frame(cbind(x,m_x))     # Combines by columns a sequence of vector, matrix or dataframe.Binds m_x and x by columns. It exists also rbind, it binds the data by rows.


# lifetable() function


lifetable(fr.mort,years=2000,type="period")  # create a life table - cross-section

# Create a life table  with selected columns and personalized names 

z<-extract.years(fr.mort,2006)                                          # demogdata object for france mortality for the year 2006 
a<-lifetable(z)                                                         # generate life tables from z 
names(a)
w<-data.frame(a$age,a$mx,a$qx,a$lx,a$Lx,a$Tx,a$ex)                      # create a data frame with only selected columns
names(w)<-c("Age","Mortality death rate",                               # rename the columns of w
            "Probability of death","Suvivers at age x",
            "Person years lived at age x",
            "Total number of years lived from age x",
            "Life expectancy at age x")            
View(w)

# Cohort reconstructed from age values at each year in the data series

lifetable(fr.mort,type="period", max.age=10, years =1900:1905)  # create life tables from 0 to 10 years old for the years from 1900 to 1905.

# life.expectancy function 


life.expectancy(fr.mort)                 # Time-Series object - life expectancy at birth

plot(life.expectancy(fr.mort),ylab="Life expectancy", xlab="Year", main="Life expectancy", col="red")   # plot is a generic function for plotting of R objects. Here we plot life.expectancy from demography package.


trend<-as.data.frame(life.expectancy(fr.mort,type='period',age=0))     # life expectancy at birth for cross-section of time. 

plot(trend)

# Create a function to compute a life table: looping over columns and rows 

my_lifetable<- function(input) {
  
  for (i in 1:nrow(input)) {
    if(i==1) {
      input[1,c("l_x")]<-100000 # set to 100000 
    }
    else {
      input[i,c("l_x")]<-round(input[i-1,c("l_x")]-input[i-1,c("m_x")]*input[i-1,c("l_x")])  # lx= survivers at year x-1 - deaths at year x-1=  lx-1 - (lx-1*mx)
    }
  }
...
# For the first 2 rows: if age = 0 than  100000 else 100000 - 100000*0.004510
...
life_data <- lt %>%
  mutate(Lx = (l_x + lag(l_x, default = 0))/2) %>% 
  mutate(Tx= sum(Lx)-lag(Lx, default = 0))

# Calculate Tx
Tx <- sum(life_data$Lx)

# Calculate ex
ex <- Tx / life_data$lx[1]


# Calculate ex (life expectancy)
ex <- sum(life_table_data$Lx) / life_table_data$survivors[1]



# EXERCISE 1.1 Complete calculation of complete life table from the function ----
