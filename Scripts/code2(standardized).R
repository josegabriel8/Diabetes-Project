...
## Matrix operations and data structure adjustment ----

# Create 2 matrices with the same dimension  

x <- matrix(c(2,5,6,7,98,3,5,9), nrow = 2)
y<-matrix(c(4,5,7,9,9,4,6,8), nrow = 2)

# The operations can be easily computed on the matrices of the same shape
x/y

# If you have to deal with different dimensions, you need to adjust the data structure
...
Standard_matrix<-matrix(c(rep(0,6)),3)  # Create a 3x2 matrix with 0 entries.   
...
x_adj<-merge(as.data.frame.matrix(x),as.data.frame.matrix(Standard_matrix),by="row.names",all.y=TRUE)
x_adj<-x_adj[2:3]
x_adj[is.na(x_adj)] <- 0
...
ny_hospdata$recl_age<-ifelse(ny_hospdata$cl_age %in% c(1,2,3),3,ny_hospdata$cl_age) # Creates a new variable assigning to class 3, the age classes 1,2,3 of cl_age variable

# Create a common structure for the denominator/nominator
n_terms<-length(levels(as.factor(ny_hospdata$recl_age)))*length(levels(as.factor(ny_hospdata$males)))
std_m<-matrix(c(rep(0,1,n_terms)),length(levels(as.factor(ny_hospdata$recl_age))))
rownames(std_m) <- levels(as.factor(ny_hospdata$recl_age))
colnames(std_m) <- levels(as.factor(ny_hospdata$males))


# Population table for the whole population
pop_ij<-table(ny_hospdata$recl_age,ny_hospdata$males)   # Contingency table for Age Class and gender
...
target_group<-subset(ny_hospdata,ny_hospdata$ny_hosp_id=="19")          # Select from amidata only patients of hosp_id 19
outcome_intarget<-subset(target_group,target_group$dead==1)     # Select from patients of hosp_id 19 only the expired ones 

label_target_group<-"Hospital 19"                               # Store the label name
...
rownames(den_data) <- levels(as.factor(ny_hospdata$recl_age))
colnames(den_data) <- levels(as.factor(ny_hospdata$males))
...
### EXERCISE 2.1 ----
# DIRECT STANDARDIZATION: 
# Using the code to obtain standardized rates on Hospital 19, generalize the approach to produce a table with results for all hospitals simultaneously.
# Hint: you can use a function for a loop, or vectorize the approach (preferred) using matrix operations
...
DLogit<-glm(dead~cl_age+males,family = binomial("logit"),data=diadata)
...
# Total deaths from ami patients divided by the number of observations.Average outcome in the entire sample:reference population rate

Ybar<-sum(diadata$dead,na.rm=TRUE)/dim(diadata)[1]                   # Overall death rate 
...
O1<-mean(diadata[diadata$ny_hosp_id=="1",c("dead")],na.rm=TRUE)     # Oi=(1/ni )sum(Yij ) OBSERVED DEATH RATE at hosp_id 1
...
output2<-data.frame("HOSPITAL 1",round(O1*100,2),round(rar1*100,2),N1,round(rar1.ll95*100,2),round(rar1.ul95*100,2))
names(output2)<-c("Unit","CR","SR","N","Lower95","Upper95")   # Rename the variables

# View output
output2

## Funnel Plots using the NY data ----

m_dia<-aggregate(diadata$dead,by=list(diadata$ny_hosp_id),FUN="mean")             
m_dia_nrow<-aggregate(diadata$dead,by=list(diadata$ny_hosp_id),FUN=NROW)          
...
funnel_plot(title="Acute Myocardial Infarction Mortality Rates\n in the State of New York - Year 2017", # Graph Title
            ...
            colour_var=z_dia$county_up,            # variable for colouring groups (numeric)
            ...
            graph="ami_mortality",                 # name of graph object
...
### EXERCISE 2.2 ----
# Create a function to compute Risk Adjusted Rates for AMI Mortality by Hospital
# Produce funnel plots from NY dataset using the table obtained above. Replicate the approach for a different outcome.
...
amidata$d_risky<-0
amidata$d_risky<-ifelse(amidata$risky>=2,1,amidata$d_risky)
...
emplogit(amidata$cost,amidata$dead,xlab="Cost",ylab="Logit(Dead)")       # Cost - logit(dead) plot

# Simple base model of amidata, Age, Sex and risk

Logit_reduced<-glm(dead~cl_age+males+d_risky,family = binomial("logit"),data=amidata)
...
Logit_full<-glm(dead~cl_age+males+surgical+d_risky+d_severe+ptca,family = binomial("logit"),data=amidata)
...
amidata$p<- predict(Logit_reduced,newdata=amidata,type="response")      # Create a variable with prediction probabilities from the fitted model
predicted.value <- ifelse(amidata$p> 0.5,1,0)                           # Create vector with 1 entry if prediction probability higher than 0.5 and 0 otherwise
discordant <- mean(predicted.value != amidata$dead,na.rm=TRUE)          # Nr. of discordances between observed and expected values divided by nr of observations     
...
### EXERCISE 2.3 : ----
# Apply  different models, compute the accuracy and assess the goodness of 
